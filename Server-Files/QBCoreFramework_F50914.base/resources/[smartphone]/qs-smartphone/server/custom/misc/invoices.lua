RegisterServerCallback('qs-smartphone:server:GetInvoices', function(source, cb)
    if Config.Framework == 'esx' then
                ...
                for k, v in pairs(invoices) do
                    if v.status == 'unpaid' then
                        ...
                        if Ply ~= nil then
                            ...
                            if resultNumber and resultNumber[1] then
                                print(json.encode(json.decode(resultNumber[1].charinfo).phone))
                                v.number = json.decode(resultNumber[1].charinfo).phone
                            end
                        end
                        v.label = v.notes
        ...
        elseif Config.billingSystem == 'codem-billing' then
            local invoices = MySQL.Sync.fetchAll('SELECT * FROM codem_billing WHERE targetidentifier = @targetidentifier', { ['@targetidentifier'] = Player.identifier })
            if invoices[1] ~= nil then
                for k, v in pairs(invoices) do
                    if v.status == 'unpaid' then
                        local Ply = ESX.GetPlayerFromIdentifier(v.identifier)
                        v.number = ''
                        if Ply ~= nil then
                            v.number = QS.GetPlayerFromId(source).PlayerData.charinfo.phone
                        else
                            local resultNumber = MySQL.Sync.fetchAll('SELECT charinfo FROM ' .. userColumns .. ' WHERE identifier = @id', { ['@id'] = v.identifier })
                            if resultNumber and resultNumber[1] then
                                v.number = json.decode(resultNumber[1].charinfo).phone
                            end
                        end
                        v.label = v.invoicelabel
    ...
    elseif Config.Framework == 'qb' then
        ...
        elseif Config.billingSystem == 'codem-billing' then
            ...
            if invoices[1] ~= nil then
                for k, v in pairs(invoices) do
                    if v.status == 'unpaid' then
                        local Ply = QBCore.Functions.GetPlayerByCitizenId(v.identifier)
                        v.number = ''
                        if Ply ~= nil then
                            v.number = Ply.PlayerData.charinfo.phone
                        else
                            local resultNumber = MySQL.Sync.fetchAll('SELECT charinfo FROM ' .. userColumns .. ' WHERE citizenid = @citizenid', { ['@citizenid'] = v.identifier })
                            if resultNumber and resultNumber[1] then
                                v.number = json.decode(resultNumber[1].charinfo).phone
                            end
                        end
