...
function switchTenu(color)
	...
	if not isCostumed then -- Si n' pas sa tenue
		TriggerEvent("inventory:client:DisableInventory", source, true)
		...
		playerTenu[9].model = GetPedDrawableVariation(ped,9)
		playerTenu[9].color = GetPedTextureVariation(ped,9)
		if sex == "men" then
			if color == "blue" then
				isCostumed = true
		...
		TriggerEvent("inventory:client:DisableInventory",  source, false)
		isCostumed = false
