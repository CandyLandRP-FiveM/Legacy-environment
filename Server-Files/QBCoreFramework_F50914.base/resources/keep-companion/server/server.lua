...
local function remove_item(src, Player, name, amount)
    ...
    if res then
        TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items[name], "remove")
    end
    return res
end
...
QBCore.Functions.CreateCallback('keep-companion:server:decrease_thirst', function(source, cb, data)
    local player = QBCore.Functions.GetPlayer(source)
    local pet_water_bottle = player.Functions.GetItemByName(Config.core_items.water_bottle.item_name)
