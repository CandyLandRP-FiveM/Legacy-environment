...
RegisterNetEvent("fs:selector:show", function(data)
    if data then
        BuildSelector(data);
    else
        TriggerServerEvent("fs:selector:show")
    end
end)
...
RegisterNetEvent("fs-spawnselector:set")
AddEventHandler("fs-spawnselector:set" , function()
    BuildSelector('');
end)
