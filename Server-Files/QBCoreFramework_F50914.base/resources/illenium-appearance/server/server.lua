...
lib.callback.register("illenium-appearance:server:payForTattoo", function(source, tattoo)
    ...
    if Framework.RemoveMoney(src, "cash", cost) then
        TriggerClientEvent('okokNotify:Alert', source, 'Success', 'Purchased %s tattoo for %s$', 5000, 'success', playSound)
        return true
    else
        TriggerClientEvent('okokNotify:Alert', source, 'Tattoo apply failed', 'You do not have enough money!', 5000, 'error', playSound)
        return false
    end
end)
...
RegisterNetEvent("illenium-appearance:server:updateOutfit", function(id, model, components, props)
    ...
    if model and components and props then
        ...
        TriggerClientEvent('okokNotify:Alert', source, 'Success', 'Outfit %s has been updated', 5000, 'success', playSound)
    end
end)
...
RegisterNetEvent("illenium-appearance:server:saveManagementOutfit", function(outfitData)
    ...
    TriggerClientEvent('okokNotify:Alert', source, 'Success', 'Outfit %s has been saved', 5000, 'success', playSound)
end)
    ...
    lib.addCommand("pedmenu", {
        ...
        if args.playerID then
            ...
            if citizenID then
                ...
                TriggerClientEvent('okokNotify:Alert', source, 'Error', 'Player not online', 5000, 'error', playSound)
                return
            end
        end
