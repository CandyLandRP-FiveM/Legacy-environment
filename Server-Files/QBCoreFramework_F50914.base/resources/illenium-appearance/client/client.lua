...
function OpenShop(config, isPedMenu, shopType)
    lib.callback("illenium-appearance:server:hasMoney", false, function(hasMoney, money)
        if not hasMoney and not isPedMenu then
            ...
            exports['okokNotify']:Alert('Cannot Enter Shop', 'Not enough cash. Need $' .. money, 5000, 'error', playSound)
            return
        end
        ...
        client.startPlayerCustomization(function(appearance)
            if appearance then
                ...
                exports['okokNotify']:Alert('Cancelled Customization', 'Customization not saved', 5000, 'info', playSound)
            end
            Framework.CachePed()
        end, config)
    end, shopType)
...
RegisterNetEvent("illenium-appearance:client:importOutfitCode", function()
    ...
    if outfitCode ~= nil then
        ...
        lib.callback("illenium-appearance:server:importOutfitCode", false, function(success)
            if success then
                exports['okokNotify']:Alert('Outfit Imported', 'You can now change to the outfit using the outfit menu', 5000, 'success', playSound)
            else
                exports['okokNotify']:Alert('Import Failure', 'Invalid outfit code', 5000, 'error', playSound)
            end
...
RegisterNetEvent("illenium-appearance:client:generateOutfitCode", function(id)
    lib.callback("illenium-appearance:server:generateOutfitCode", false, function(code)
        if not code then
            exports['okokNotify']:Alert('Something went wrong', 'Code generation failed for the outfit', 5000, 'error', playSound)
            return
        end
    ...
    if outfitName then
        ...
        lib.callback("illenium-appearance:server:getOutfits", false, function(outfits)
            ...
            if outfitExists then
                exports['okokNotify']:Alert('Save Failed', 'Outfit with this name already exists', 5000, 'error', playSound)
                return
            end
...
RegisterNetEvent('illenium-appearance:client:updateOutfit', function(outfitID)
    ...
    lib.callback("illenium-appearance:server:getOutfits", false, function(outfits)
        ...
        if not outfitExists then
            exports['okokNotify']:Alert('Update Failed', 'That outfit does not exist', 5000, 'error', playSound)
            return
        end
    ...
    if pedModel ~= data.model then
        ...
        lib.callback("illenium-appearance:server:getAppearance", false, function(appearance)
            ...
            if appearance then
                ...
                exports['okokNotify']:Alert('Something went wrong', 'The outfit that you are trying to change to, does not have a base appearance', 5000, 'error', playSound)
            end
...
RegisterNetEvent("illenium-appearance:client:DeleteManagementOutfit", function(id)
    ...
    exports['okokNotify']:Alert('Success', 'Outfit Deleted', 5000, 'success', playSound)
end)
...
RegisterNetEvent("illenium-appearance:client:reloadSkin", function(bypassChecks)
    if not bypassChecks and InCooldown() or Framework.CheckPlayerMeta() or cache.vehicle or IsPedFalling(cache.ped) then
        exports['okokNotify']:Alert('Error', 'You cannot use reloadskin right now', 5000, 'error', playSound)
        return
    end
...
RegisterNetEvent("illenium-appearance:client:ClearStuckProps", function()
    if InCooldown() or Framework.CheckPlayerMeta() then
        exports['okokNotify']:Alert('Error', 'You cannot use clearstuckprops right now', 5000, 'error', playSound)
        return
    end
