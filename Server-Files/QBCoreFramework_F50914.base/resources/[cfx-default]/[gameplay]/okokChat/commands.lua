	...
	RegisterCommand(Config.ClearEveryonesChatCommand, function(source, args, rawCommand)
		...
		if isAdmin(source) then
			...
			TriggerClientEvent('chat:addMessage', -1, {
				template = '<div class="chat-message system"><i class="fas fa-cog"></i> <b><span style="color: #df7b00">SYSTEM</span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{0}</span></b><div style="margin-top: 5px; font-weight: 300;">Der Chat wurde gelöscht!</div></div>',
				args = { time }
			})
		end
	end)
...
if Config.EnableStaffCommand then
	RegisterCommand(Config.StaffCommand, function(source, args, rawCommand)
		...
		if isAdmin(source) then
			TriggerClientEvent('chat:addMessage', -1, {
				template = '<div class="chat-message staff"><i class="fas fa-shield-alt"></i> <b><span style="color: #bc1e29">[ADMIN] {0}</span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{2}</span></b><div style="margin-top: 5px; font-weight: 300;">{1}</div></div>',
		...
		if isAdmin(source) then
			showOnlyForAdmins(function(admins)
				TriggerClientEvent('chat:addMessage', admins, {
					template = '<div class="chat-message staffonly"><i class="fas fa-eye-slash"></i> <b><span style="color: #1ebc62">[ADMIN CHAT] {0}</span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{2}</span></b><div style="margin-top: 5px; font-weight: 300;">{1}</div></div>',
					args = { playerName, message, time }
				})
			end)
		end
	...
	RegisterCommand(Config.AdvertisementCommand, function(source, args, rawCommand)
		...
		if canAdvertise then
			if bankMoney >= Config.AdvertisementPrice then
				...
				TriggerClientEvent('okokNotify:Alert', source, "WERBUNG", "Werbung erfolgreich gemacht für "..Config.AdvertisementPrice..'€', 10000, 'success')
				...
				TriggerClientEvent('okokNotify:Alert', source, "WERBUNG", "Sie haben nicht genug Geld, um eine Anzeige zu machen", 10000, 'error')
			end
		else
			TriggerClientEvent('okokNotify:Alert', source, "WERBUNG", "So schnell kann man nicht werben", 10000, 'error')
		end
	end)
end
