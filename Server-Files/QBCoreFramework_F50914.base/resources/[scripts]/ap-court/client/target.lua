...
Citizen.CreateThread(function()
  ...
  if Config.Interactions.qbTarget then
    ...
    if qbtargetcfg['lawyer'].active == true then
      local input = {
        {
          type = "client",
          ...
          job = {[qbtargetcfg['lawyer'].job.name] = qbtargetcfg['lawyer'].job.grade}
        },
        {
          type = "client",
      ...
      if Config.CriminalRecordArchives == true then
        table.insert(input, {type = "client", event = "ap-court:client:criminalRecordArc", icon = "fas fa-sign-in-alt", label = "Search Archives", job = {[qbtargetcfg['lawyer'].job.name] = qbtargetcfg['lawyer'].job.grade}})
      end
      exports['qb-target']:AddBoxZone(qbtargetcfg['lawyer'].boxzone.name, qbtargetcfg['lawyer'].boxzone.coords, qbtargetcfg['lawyer'].boxzone.length, qbtargetcfg['lawyer'].boxzone.width, {
        name=qbtargetcfg['lawyer'].boxzone.name,
        heading=qbtargetcfg['lawyer'].boxzone.heading,
        debugPoly=qbtargetcfg['lawyer'].boxzone.debugPoly, 
        minZ=qbtargetcfg['lawyer'].boxzone.minZ,
        maxZ=qbtargetcfg['lawyer'].boxzone.maxZ,
        }, {
          options = input,
        distance = 2.5
      })
...
Citizen.CreateThread(function()
  ...
  if Config.Interactions.qTarget then
    ...
    if qtargetcfg['lawyer'].active == true then
      local input = {
        {
          event = "ap-court:client:backgroundChecks",
          icon = "fas fa-sign-in-alt",
      ...
      exports['qtarget']:AddBoxZone(qtargetcfg['lawyer'].boxzone.name, qtargetcfg['lawyer'].boxzone.coords, qtargetcfg['lawyer'].boxzone.length, qtargetcfg['lawyer'].boxzone.width, {
        name=qtargetcfg['lawyer'].boxzone.name,
        heading=qtargetcfg['lawyer'].boxzone.heading,
        debugPoly=qtargetcfg['lawyer'].boxzone.debugPoly, 
        minZ=qtargetcfg['lawyer'].boxzone.minZ,
        maxZ=qtargetcfg['lawyer'].boxzone.maxZ,
        }, {
          options = input,
        job = {[qtargetcfg['lawyer'].job.name] = qtargetcfg['lawyer'].job.grade},
        distance = 2.5
      })
    end
