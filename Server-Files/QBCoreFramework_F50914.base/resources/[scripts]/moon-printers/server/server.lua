    ...
    if url ~= nil then
        if validexts[extension] then
            ...
            if cash >= Config.Amount then 
                if Player.Functions.RemoveItem('printerpaper', 1) then
                    ...
                    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['printerpaper'], "remove")
                ...
                TriggerClientEvent('QBCore:Notify', source, 'You dont have enough money'..Config.Amount..'costs', 'error', 2500) 
            end
        else
